...
class PasswordVC: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func saveBtnPressed(_ sender: Any) {
        ...
        if let currentPwd = currentPwdLbl.text, currentPwd != "", let newPwd = newPwdLbl.text, newPwd != "", currentPwd.count >= 5, newPwd.count >= 5 {
            
            swiftLoader()
            checkForCurrentPwd(currentInput: currentPwd, newInput: newPwd)
    ...
    func checkForCurrentPwd(currentInput: String, newInput: String) {
            ...
            for item in snap!.documents {
                ...
                if let pwd = i["password"] as? String {
                    ...
                    if currentInput == pwd {
                        ...
                        DataService.instance.mainFireStoreRef.collection("Pwd_users").document(id).updateData(["password": newInput]) { (error) in
